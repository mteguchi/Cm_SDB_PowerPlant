...
pre.all.01 %>% group_by(ID.f) %>%
  mutate(day1 = ifelse(day1night2 == 1, 1, 0)) %>%
  mutate(night1 = ifelse(day1night2 == 2, 1, 0)) %>%
  mutate(day1a = ifelse(day1night2 == 1 & include == 1, 1, 0)) %>%
  mutate(night1a= ifelse(day1night2 == 2 & include == 1, 1, 0)) %>%
...
h.pre <- round(pre.kd$kd.href@h$h * 0.6)
...
## Section 3.1.2
##Visually optimized hlim = c(0.565,1.5), grid=300
h.post <- round(post.kd$kd.href@h$h * 0.4)
...
```{r}
h.pre.day <- round(pre.kd.day$kd.href@h$h * 0.7)
...
```{r}
h.pre.night <- round(pre.kd.night$kd.href@h$h * 0.5)
# or use the same value as before
...
Bring in the body size information. This was done in deployment_table.R script. 
```{r}
dat.table <- read.csv(paste0("data/all_sample_summary_size_",
                   file.date, ".csv"))
