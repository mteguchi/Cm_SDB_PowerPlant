...
post.raw %>%
  filter(include == 1 & inside == 1) -> post.all
...
new.raw %>% 
  filter(include == 1 & inside == 1) -> new.all
...
write.csv(dat.table, file = "data/IDvsUDs.csv", quote = F, row.names = F)
...
p.h.post.eachID <- ggplot() + 
  ...
  scale_y_continuous(breaks = c(32.60, 32.62, 32.64, 32.66, 32.68),
                     limits = c(32.59, 32.70))
