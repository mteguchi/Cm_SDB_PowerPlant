...
# 
area.pre.daynight.95 <- spTransform(get.area.UTM(pre.daynight$HR$ver.95),
...
area.pre.night.95 <- spTransform(get.area.UTM(pre.night$HR$ver.95),
                                 CRS=CRS("+proj=longlat +datum=WGS84"))
area.pre.daynight.50 <- spTransform(get.area.UTM(pre.daynight$HR$ver.50),
                                    CRS=CRS("+proj=longlat +datum=WGS84"))
...
area.pre.daynight.95.byID <- lapply(pre.daynight.byID$UD.95, 
                                    FUN = function(x) spTransform(x, 
                                                                  CRS=CRS("+proj=longlat +datum=WGS84"))) 
area.pre.day.95.byID <- lapply(pre.day.byID$UD.95, 
                               FUN = function(x) spTransform(x, 
                                                             CRS=CRS("+proj=longlat +datum=WGS84"))) 
area.pre.night.95.byID <- lapply(pre.night.byID$UD.95, 
                                 FUN = function(x) spTransform(x, 
                                                               CRS=CRS("+proj=longlat +datum=WGS84"))) 
area.pre.daynight.50.byID <- lapply(pre.daynight.byID$UD.50, 
                                    FUN = function(x) spTransform(x, 
                                                                  CRS=CRS("+proj=longlat +datum=WGS84"))) 
area.pre.day.50.byID <- lapply(pre.day.byID$UD.50, 
                               FUN = function(x) spTransform(x, CRS=CRS("+proj=longlat +datum=WGS84")))      
area.pre.night.50.byID <- lapply(pre.night.byID$UD.50, 
                                 FUN = function(x) spTransform(x, CRS=CRS("+proj=longlat +datum=WGS84"))) 

area.post.daynight.95 <- spTransform(get.area.UTM(post.daynight$HR$ver.95),
                                    CRS=CRS("+proj=longlat +datum=WGS84"))
area.post.day.95 <- spTransform(get.area.UTM(post.day$HR$ver.95),
                                    CRS=CRS("+proj=longlat +datum=WGS84"))
area.post.night.95 <- spTransform(get.area.UTM(post.night$HR$ver.95),
                                    CRS=CRS("+proj=longlat +datum=WGS84"))
 
area.post.daynight.50 <- spTransform(get.area.UTM(post.daynight$HR$ver.50),
                                    CRS=CRS("+proj=longlat +datum=WGS84"))
area.post.day.50 <- spTransform(get.area.UTM(post.day$HR$ver.50),
                                    CRS=CRS("+proj=longlat +datum=WGS84"))
area.post.night.50 <- spTransform(get.area.UTM(post.night$HR$ver.50),
                                    CRS=CRS("+proj=longlat +datum=WGS84"))

area.post.daynight.95.byID <- lapply(post.daynight.byID$UD.95, 
                                    FUN = function(x) spTransform(x, 
                                                                  CRS=CRS("+proj=longlat +datum=WGS84"))) 

area.post.day.95.byID <- lapply(post.day.byID$UD.95, 
                           FUN = function(x) spTransform(x, 
                                                         CRS=CRS("+proj=longlat +datum=WGS84")))
area.post.night.95.byID <- lapply(post.night.byID$UD.95, 
                             FUN = function(x) spTransform(x, 
                                                           CRS=CRS("+proj=longlat +datum=WGS84")))
 
area.post.daynight.50.byID <- lapply(post.daynight.byID$UD.50, 
                                FUN = function(x) spTransform(x, 
                                                              CRS=CRS("+proj=longlat +datum=WGS84")))
area.post.day.50.byID <- lapply(post.day.byID$UD.50, 
                           FUN = function(x) spTransform(x, 
                                                         CRS=CRS("+proj=longlat +datum=WGS84")))
area.post.night.50.byID <- lapply(post.night.byID$UD.50, 
                             FUN = function(x) spTransform(x, 
                                                           CRS=CRS("+proj=longlat +datum=WGS84")))# 
...
```{r, message=FALSE}
pre.IDs <- pre.daynight.byID$area$ID
...
if (save.fig)
  ggsave(plot = p.KDE.pre.combined,
         filename = "figures/KDE_pre_combined_Jan2020.png",
         device = "png",
         dpi = 600)
```
...
```{r, message=FALSE}
post.IDs <- post.daynight.byID$area$ID
...
p.KDE.post.combined <- ggplot() + 
  ...
  geom_polygon(data = post.95.byID.df,
               aes(x = long, y = lat,
                   group = group),
               color = post.95.color, #"black",
               fill = post.95.fill, #"mediumpurple1",
               alpha = 0.4) +

  geom_polygon(data = post.50.byID.df,
               aes(x = long, y = lat,
                   group = group),
               color =  post.50.color, #"black",
               fill = post.50.fill, #"mediumpurple4",
               alpha = 0.4) +
  geom_polygon(data = SDBay.eelg.2014.df,
               aes(x = long,
                   y = lat,
                   group = group),
               fill = eelgrass.color,# "green",
               alpha = 0.5) +
   geom_polygon(data = SDBay.df,
               aes(x = long, y = lat, group = group),
               fill = "transparent",
...
if (save.fig)
  ggsave(plot = p.KDE.post.combined,
         filename = "figures/KDE_post_combined_Jan2020.png",
         device = "png",
         dpi = 600)
```
...
```{r}
pre.day.IDs <- pre.day.byID$area$ID
...
p.KDE.pre.day <- ggplot() + 
  ...
  geom_polygon(data = SDBay.eelg.2008.df,
               aes(x = long,
                   y = lat,
                   group = group),
               fill = eelgrass.color, #"mediumseagreen",
               alpha = 0.5) +
  geom_polygon(data = pre.day.95.byID.df,
               aes(x = long, y = lat,
                   group = group),
               color = pre.95.color, #"black",
               fill = pre.95.fill, #"lightcyan",
               alpha = 0.6) +

  geom_polygon(data = pre.day.50.byID.df,
               aes(x = long, y = lat,
                   group = group),
               color = pre.50.color, # "black",
               fill = pre.50.fill, #"blue4",
               alpha = 0.6) +
    geom_polygon(data = SDBay.df,
               aes(x = long, y = lat, group = group),
               fill = "transparent",
               color = "black") +
  coord_map(xlim = map.west.east, # c(-117.145, -117.09),
            ylim = map.north.south) + 
  facet_grid(. ~ ID) +
  facet_wrap( ~ ID, nrow = 2) + 
  labs(x = "", y = "")+ 
...
if (save.fig)
  ggsave(plot = p.KDE.pre.day,
         filename = "figures/KDE_pre_day_Jan2020.png",
         device = "png",
         dpi = 600)
...
```{r}
pre.night.IDs <- pre.night.byID$area$ID
...
p.KDE.pre.night <- ggplot() + 
  ...
  # scale_y_continuous(breaks = c(32.60, 32.62, 32.64),
  #                    limits = c(32.595, 32.665))
plot(p.KDE.pre.night)

if (save.fig)
  ggsave(plot = p.KDE.pre.night,
         filename = "figures/KDE_pre_night_Jan2020.png",
         device = "png",
         dpi = 600)
...
```{r}
post.day.IDs <- post.day.byID$area$ID
...
if (save.fig)
  ggsave(plot = p.KDE.post.day,
         filename = "figures/KDE_post_day_Jan2020.png",
         device = "png",
         dpi = 600)
...
Post period night time. 

```{r}
post.night.IDs <- post.night.byID$area$ID
...
p.KDE.post.night <- ggplot() + 
  ...
  #                  group = group),
  #              color = post.50.color, # "black",
  #              fill = post.50.fill, #"blue4",
  #              alpha = 0.6) +
  geom_polygon(data = post.night.95.byID.df,
               aes(x = long, y = lat,
                   group = group),
               color = post.95.color, #"black",
               fill = post.95.fill, #"lightpink",
               alpha = 0.5) +
...
if (save.fig)
  ggsave(plot = p.KDE.post.night,
         filename = "figures/KDE_post_night_Jan2020.png",
         device = "png",
         dpi = 600)

plot(p.KDE.post.night)
```
...
#          filename = "figures/KDE_pre_post_day_night_Jan2020.png",
#          device = "png",
#          dpi = 600)
# 
...
p.KDE.pre.all <- ggplot() + 
  ...
  geom_polygon(data = pre.daynight.95,
               aes(x = long, y = lat,
                   group = group),
               color = pre.95.color, #"black",
               fill = pre.95.fill, #"lightgoldenrod1",
               alpha = 0.6) +
  geom_polygon(data = pre.daynight.50,
               aes(x = long, y = lat,
                   group = group),
               color = pre.50.color, # "black",
               fill = pre.50.fill, #"gold",
               alpha = 0.6) +
   geom_polygon(data = SDBay.df,
               aes(x = long, y = lat, group = group),
               fill = "transparent",
...
#          filename = "figures/KDE_pre_all_Jan2020.png",
#          device = "png",
#          dpi = 600)
...
p.KDE.pre.post.all <- ggplot() + 
  ...
  geom_polygon(data = post.daynight.95,
               aes(x = long, y = lat,
                   group = group),
               color = post.95.color, #"black",
               fill = post.95.fill, #"lightgreen",
               alpha = 0.5) +
  geom_polygon(data = post.daynight.50,
               aes(x = long, y = lat,
                   group = group),
               color = post.50.color, #"black",
               fill = post.50.fill, #"green1",
               alpha = 0.5) +
   geom_polygon(data = SDBay.df,
               aes(x = long, y = lat, group = group),
               fill = "transparent",
...
#          filename = "figures/KDE_pre_post_all_Jan2020.png",
#          device = "png",
#          dpi = 600)
...
#          filename = "figures/KDE_pre_DayVsNight_Jan2020.png",
#          device = "png",
#          dpi = 600)
...
#          filename = "figures/KDE_post_DayVsNight_Jan2020.png",
#          device = "png",
#          dpi = 600)
...
SDBay.eelg.2008.df$period.f <- as.factor("pre")
SDBay.eelg.2014.df$period.f <- as.factor("post")
SDBay.eelg.df <- rbind(SDBay.eelg.2008.df, SDBay.eelg.2014.df)
...
all.95$daynight.f <- factor(all.95$daynight,
                            levels = c("day", "night", "day+night"))
all.50$UD.f <- as.factor("50")

all.50$period.f <- factor(all.50$period,
                          levels = c("pre", "post"))

all.50$UD.period <- as.factor(paste0(all.50$period.f, all.50$UD.f))

all.95$period.f <- factor(all.95$period,
                          levels = c("pre", "post"))
all.95$UD.f <- as.factor("95")
all.95$UD.period <- as.factor(paste0(all.95$period.f, all.95$UD.f))

# Because the class of all.95 and all.50 are "tbl_df", "tbl" and "data.frame",
# using scalebar returns an error message: "Must use a vector in '[', not an object of 
# class matrix... so force it to become just data frame. 
all.50 <- as.data.frame(all.50)
all.95 <- as.data.frame(all.95)
all. <- rbind(all.50, all.95)
...
p.KDE.pre.post.DayVsNight <- ggplot() + 
  geom_polygon(data = SDBay.df,
               ...
               fill = water.color,
               color = "black") +
  geom_polygon(data = SDBay.eelg.df,
               aes(x = long,
                   y = lat,
                   group = group),
               fill = eelgrass.color,
  ...
  geom_polygon(data = all.50,
               aes(x = long, y = lat,
                   group = group,
                   color = UD.period, #post.50.color, #"black",
                   fill = UD.period), #post.50.fill, #"green1",
               alpha = 0.6) +
  geom_polygon(data = SDBay.df,
               aes(x = long, y = lat, group = group),
               fill = "transparent",
  ...
  #                y.min = 32.6,
  #                y.max = 32.62,
  #                facet.var = c("daynight.f", "period.f"),
  #                facet.lev = c("day+night", "pre")) +
  facet_grid(daynight.f ~ period.f) +
  # facet_wrap( ~ ID, nrow = 2) + 
  labs(x = "", y = "")+ 
  scale_color_manual(values = c("post50" = post.50.color,
                                "post95" = post.95.color,
                                "pre50" = pre.50.color,
                                "pre95" = pre.95.color)) + 
  scale_fill_manual(values = c("post50" = post.50.fill,
                                "post95" = post.95.fill,
                                "pre50" = pre.50.fill,
                                "pre95" = pre.95.fill)) + 
  theme(panel.border = element_rect(color = "black", fill = NA),
  ...
  scale_x_continuous(breaks = c(-117.15, -117.13, -117.11, -117.09)) 
# scale_y_continuous(breaks = c(32.60, 32.62, 32.64, 32.66),
#                    limits = c(32.595, 32.665))

p.KDE.pre.post.DayVsNight
if (save.fig)
  ggsave(plot = p.KDE.pre.post.DayVsNight,
         filename = "figures/KDE_preVspost_DayVsNight_Jan2020.png",
         device = "png",
         dpi = 600)
