...
```{r}
ggplot(data = filter(area.df, PP == 0)) + 
  geom_point(aes(x = wtemp, y = lat.center, color = as.factor(ID)))
```

```{r}
ggplot(data = filter(area.df, PP == 1)) + 
  geom_point(aes(x = wtemp, y = lat.center, color = as.factor(ID)))
```
...
```{r}
library(lme4)
fit.1 <- lmer(area ~ PP + wtemp + SD_wtemp| ID, data = area.df)
fit.2 <- lmer(area ~ wtemp + SD_wtemp| ID, data = area.df) 
fit.3 <- lmer(area ~ PP +  SD_wtemp| ID, data = area.df)
fit.4 <- lmer(area ~ PP + wtemp | ID, data = area.df) 
fit.5 <- lmer(area ~ wtemp | ID, data = area.df)
fit.6 <- lmer(area ~ SD_wtemp| ID, data = area.df)
