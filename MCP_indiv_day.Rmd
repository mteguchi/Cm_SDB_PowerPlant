...
min_n <- 50
n_days <- 10   # # days pooled to create a MCP
MCP.percent <- 95
...
library(lme4)
#library(mgcv)
fit.1 <- lmer(lat.center ~ PP + wtemp + wtemp_7 + wtemp_14 + SD_wtemp + (1 | ID), 
              data = area.df)

fit.2 <- lmer(lat.center ~ PP + wtemp + wtemp_7 + wtemp_14 + (1 | ID), 
              data = area.df) 
fit.3 <- lmer(lat.center ~ PP +  wtemp_7 + wtemp_14 + (1 | ID), 
              data = area.df)
fit.4 <- lmer(lat.center ~ wtemp_7 + wtemp_14 + (1 | ID), 
              data = area.df) 
fit.5 <- lmer(lat.center ~ wtemp_14 + (1 | ID), 
              data = area.df)
fit.6 <- lmer(lat.center ~ wtemp_7 + (1 | ID), 
              data = area.df)
fit.7 <- lmer(lat.center ~ wtemp + (1 | ID), 
              data = area.df)
 
anova(fit.1, fit.2, fit.3, fit.4, fit.5, fit.6, fit.7)
summary(fit.4)
fit.list <- list(fit.1, fit.2, fit.3, fit.4, fit.5, fit.6, fit.7)
