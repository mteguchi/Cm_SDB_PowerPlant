...
```{r}
post.GPS.kd <- HR.analysis.step1(min_n, post.summary$GPS, tagproj, grid = grid.value)
post.ARGOS.kd <- HR.analysis.step1(min_n, post.summary$ARGOS, tagproj, grid = grid.value)
```
...
```{r}
plotLSCV(post.ARGOS.kd$kd.LSCV)
...
Somehow, only 6 post animals get UD computed... need to look into why that is 2019-11-04...  
```{r}
