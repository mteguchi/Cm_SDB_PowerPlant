...
for (d in 1:length(dNames)){
  ...
  if (!file.exists(paste0(dNames[d], "/", IDs[1], '_inside_DayNight_',
                            Sys.Date(), '.csv'))){
    ...
    for (k in 1:length(filtered.rise.set)){
      ...
      if (!file.exists(paste0(dNames[d], "/", IDs[k], '_inside_DayNight_',
                              Sys.Date(), '.csv'))){
        outfile <- paste0(dNames[d], "/", IDs[k], '_inside_DayNight_',
                          Sys.Date(), '.csv')
...
d <- k0 <- k1 <- k2 <- 1
for (d in 1:length(dNames)){
  all.files <- dir(dNames[d], "_DayNight_")
  
  for (k0 in 1:length(all.files)){
    id <- unlist(strsplit(all.files[k0], '_'))[1]
    
    fileout.gps <- paste0(dNames[d], '/', id, "_DayNight_4hrs_GPS_",
                          Sys.Date(), ".csv")

    fileout.argos <- paste0(dNames[d], '/', id, "_DayNight_4hrs_ARGOS_",
                          Sys.Date(), ".csv")

    if (!file.exists(fileout.gps)){
