...
ARGOS data are not quite the same. 
...
if (save.figure)
  ggsave(filename = "figures/map_pre_ARGOS_h_adhoc_day.png",
         plot = pre.ARGOS.HR.day$h.adhoc$figure,
         device = "png",
         dpi = 600)
...
For the ARGOS data, h = 216.1 and the minimum muliplier is 0.7. 
...
if (save.figure)
  ggsave(filename = "figures/map_pre_ARGOS_h_adhoc_night.png",
         plot = pre.ARGOS.HR.night$h.adhoc$figure,
         device = "png",
         dpi = 600)
```
...
See post.ARGOS.HR.night$h.1 
Estimated h is 229.8 and the minimum multiplier of 0.85.
...
Now, I need to look at the effects of various things on the HR sizes.  Maybe good to remove everything else and start fresh here.

The first comparison is the main one; pre vs. post, where using day and night data together.  I use only GPS data first. 
...
lm.fit.all.50 <- lm(formula = area.50.all.GPS ~ period + CCL_cm + n.days.GPS + n.relocations.all.GPS,
                    data = table.area.50.GPS)

summary(lm.fit.all.50)
```

The model indicates no significant effects of the period (uncertainty is too big). Longer deployments result in larger UDs, whereas larger turtles tend to have smaller UDs. 

Second, I use day and night data as separate but drop the combined data.  To account for individuals, they are treated as random effects. There are only 6 individuals available fro this analysis.  
...
area.50.day.night.GPS.long <- gather(area.50.day.night.GPS,
                                     day_night, area, area.50.night.GPS, area.50.day.GPS) 

lmm.fit.all.50 <- lmer(formula = area ~ day_night + period + CCL_cm + (1|ArgosID),
                      data = area.50.day.night.GPS.long)

summary(lmm.fit.all.50)
```

UDs are smaller during the night than the day and for the pre period than the post period. Effects of CCL is minimal in this analysis.  
...
lm.fit.all.95 <- lm(formula = area.95.all.GPS ~ period + CCL_cm + n.days.GPS + n.relocations.all.GPS,
...
area.95.day.night.GPS.long <- gather(area.95.day.night.GPS,
                                     day_night, area, area.95.night.GPS, area.95.day.GPS) 

lmm.fit.all.95 <- lmer(formula = area ~ day_night + period + CCL_cm + (1|ArgosID),
