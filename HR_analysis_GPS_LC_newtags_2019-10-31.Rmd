...
Looking at the GPS data, referecne or optimum bandwidth is ~248 (see pre.GPS.kd\$kd.href@h), whereas the LSCV comes back with h = 27.5 (pre.GPS.kd\$kd.LSCV@h). According to Kie (2013), we should decrease href in increments of 0.10 and the best value is the smallest increment of href that 1. resulted in a contiguous rather than disjoint 95% kernel home-range polygon, and 2. contained no lacuna within the home range. 
...
START FROM HERE ON 11/7/2019.  TEXT NEEDS TO BE FIXED AND ARGOS-BASED HR NEED TO BE COMPUTED.  I THINK GPS-BASED HR ANALYSES ARE DONE. 
...
for (k in 1:nrow(dat.table.pre)){
  tmp.turtle.SDB <- filter(turtle.SDB, 
                           ArgosID == dat.table.pre$ArgosID[k])
  if (nrow(tmp.turtle.SDB) == 1){
    tmp.turtle.SDB %>% transmute(ArgosID = dat.table.pre$ArgosID[k],
                                 Turtle_ID = Turtle_ID,
...
dat.table.pre <- full_join(dat.table.pre, dat.pre.morph, by = "ArgosID")%>%
  mutate(period = "pre")
...
for (k in 1:nrow(dat.table.post)){
  tmp.turtle.SDB <- filter(turtle.SDB, 
                           ArgosID == dat.table.post$ArgosID[k])
  
  if (nrow(tmp.turtle.SDB) == 1){
    tmp.turtle.SDB %>% transmute(ArgosID = dat.table.post$ArgosID[k],
                                 Turtle_ID = Turtle_ID,
                                 Capture_Date = as.Date(Capture_Date,
                                                        format = "%Y-%m-%d"),
                                 Sex = Sex,
    ...
    tmp.turtle.SDB %>% filter(Year_caught == year(dat.table.post$Date1[k])) -> tmp.turtle.SDB
    if (nrow(tmp.turtle.SDB == 1)){
      tmp.turtle.SDB %>% transmute(ArgosID = dat.table.post$ArgosID[k],
                                   Turtle_ID = Turtle_ID,
                                   Capture_Date = as.Date(Capture_Date,
...
dat.table.post <- full_join(dat.table.post, dat.post.morph, by = "ArgosID")%>%
  mutate(period = "post")
...
NOt sure why some individuals' UDs are not computed even though there are > 50 data points... Need to look into these 2019-11-06  
```{r}
dat.table <- rbind(dat.table.pre, dat.table.post) %>% 
  mutate(ID.f = ArgosID)

pre.areas.df <- data.frame(ID.f = pre.GPS.kd$list.data$unique.ID,
                           area.50 = pre.GPS.UD.byID$area$area.50,
                           area.95 = pre.GPS.UD.byID$area$area.95)

pre.day.areas.df <- data.frame(ID.f = pre.GPS.kd.day$list.data$unique.ID,
                               area.day.50 = pre.GPS.UD.day.byID$area$area.50,
                               area.day.95 = pre.GPS.UD.day.byID$area$area.95)

pre.night.areas.df <- data.frame(ID.f = pre.GPS.kd.night$list.data$unique.ID,
                                 area.night.50 = pre.GPS.UD.night.byID$area$area.50,
                                 area.night.95 = pre.GPS.UD.night.byID$area$area.95)

post.areas.df <- data.frame(ID.f = post.GPS.kd$list.data$unique.ID,
                            area.50 = post.GPS.UD.byID$area$area.50,
                            area.95 = post.GPS.UD.byID$area$area.95)

post.day.areas.df <- data.frame(ID.f = post.GPS.kd.day$list.data$unique.ID,
                                area.day.50 = post.GPS.UD.day.byID$area$area.50,
                                area.day.95 = post.GPS.UD.day.byID$area$area.95)

post.night.areas.df <- data.frame(ID.f = post.GPS.kd.night$list.data$unique.ID,
                                  area.night.50 = post.GPS.UD.night.byID$area$area.50,
                                  area.night.95 = post.GPS.UD.night.byID$area$area.95)
...
Check one that didn't have UDs for day and night combined when there were sufficient sample sizes (n > 49). ArgosID 152322 spent some time north of 32.66N, therefore those data points were removed, resulting in only 40 available data points in the end.  So, the turtle needed to be removed from further analyses. 
...
pre.ver.50.df <- broom::tidy(spTransform(pre.GPS.HR$HR$ver.50, CRS("+proj=longlat")))
pre.ver.95.df <- broom::tidy(spTransform(pre.GPS.HR$HR$ver.95, CRS("+proj=longlat")))
...
pre.day.ver.50.df <- broom::tidy(spTransform(pre.GPS.HR.day$HR$ver.50, CRS("+proj=longlat")))
pre.day.ver.95.df <- broom::tidy(spTransform(pre.GPS.HR.day$HR$ver.95, CRS("+proj=longlat")))
...
pre.night.ver.50.df <- broom::tidy(spTransform(pre.GPS.HR.night$HR$ver.50, 
                                               CRS("+proj=longlat")))

pre.night.ver.95.df <- broom::tidy(spTransform(pre.GPS.HR.night$HR$ver.95, 
                                               CRS("+proj=longlat")))
...
post.ver.50.df <- broom::tidy(spTransform(post.GPS.HR$HR$ver.50, CRS("+proj=longlat")))
post.ver.95.df <- broom::tidy(spTransform(post.GPS.HR$HR$ver.95, CRS("+proj=longlat")))
...
post.day.ver.50.df <- broom::tidy(spTransform(post.GPS.HR.day$HR$ver.50, CRS("+proj=longlat")))
post.day.ver.95.df <- broom::tidy(spTransform(post.GPS.HR.day$HR$ver.95, CRS("+proj=longlat")))
...
post.night.ver.50.df <- broom::tidy(spTransform(post.GPS.HR.night$HR$ver.50,
                                                CRS("+proj=longlat")))
post.night.ver.95.df <- broom::tidy(spTransform(post.GPS.HR.night$HR$ver.95,
                                                CRS("+proj=longlat")))
...
map.post.day.night <- map.sdbay.zm + 
  ...
  geom_polygon(data = SDBay.eelg.2008.df,
               ...
               fill = "green",
               alpha = 0.5) +
  geom_point(data = post.summary$GPS,
             aes(x = Lon1, y = Lat1, 
                 color = as.factor(day1night2)),
             alpha = 0.5)+ 
  xlab('Longitude') + ylab('Latitude') + 
...
save(list = ls(),
     file = paste0("RData/output_HR_analysis_GPS_", Sys.Date(), ".RData"))
#plot(map.post.HR)
```