...
data.files <- dir(data.dir, paste0("_inout_4hrs_", runDate, ".csv"))

for (k in 1:length(data.files)){
  dat.tmp <- read.csv(file = paste0(data.dir, data.files[k]))
  ...
  ID <- as.numeric(unlist(strsplit(data.files[k],
                                   '_DayNight_inout_4hrs'))[1])

  # these are specific to each year.
