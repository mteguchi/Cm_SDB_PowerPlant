...
Use Random Forests to find useful variables - just throw in everything! Something is not working here. RF is giving nonsensical results - look into this! 1/16/2020
...
if (!file.exists("RData/RF_M1_out.rds")){
  RF.M1 <- cforest(formula = M1,
                   data = data2, 
                   control = cforest_unbiased(ntree = 5000, trace = 500))
...
fit.lag14.50 <- lm(area.50.all.GPS ~ n.days.GPS + Mass_kg + period + tmp.lag14.mean + tmp.lag14.min + tmp.lag14.max + tmp.lag14.sd + month,
            data = data2)
...
lag 7
```{r}
fit.lag7.50 <- lm(area.50.all.GPS ~ n.days.GPS + Mass_kg + period + tmp.lag7.mean + tmp.lag7.min + tmp.lag7.max + tmp.lag7.sd + month,
            data = data2)
summary(fit.lag7.50)
...
```{r}
fit.deploy.50 <- lm(area.50.all.GPS ~ n.days.GPS + Mass_kg + period + tmp.deploy.mean + tmp.deploy.min + tmp.deploy.max + tmp.deploy.sd + month,
            data = data2)
...
```{r}
fit.middle.50 <- lm(area.50.all.GPS ~ n.days.GPS + Mass_kg + period + tmp.middle.mean + tmp.middle.min + tmp.middle.max + tmp.middle.sd + month,
            data = data2)
summary(fit.middle.50)

```

drop some 
```{r}
fit.middle.50.2 <- lm(area.50.all.GPS ~ tmp.middle.mean + tmp.middle.min + tmp.middle.max + month,
                      data = data2)
summary(fit.middle.50.2)

```

But... they are very correlated so drop two and go with min

```{r}
fit.middle.50.3 <- lm(area.50.all.GPS ~ tmp.middle.min + month,
            data = data2)
...
Just means only
```{r}
fit.means.50 <- lm(area.50.all.GPS ~ n.days.GPS + Mass_kg + period + tmp.lag7.mean + tmp.lag14.mean + tmp.deploy.mean + tmp.middle.mean + month,
            data = data2)
summary(fit.means.50)
...
Just minimum 
```{r}
fit.min.50 <- lm(area.50.all.GPS ~ n.days.GPS + Mass_kg + period + tmp.lag7.min + tmp.lag14.min + tmp.deploy.min + tmp.middle.min + month,
            data = data2)
summary(fit.min.50)
```
...
Just max
```{r}
fit.max.50 <- lm(area.50.all.GPS ~ n.days.GPS + Mass_kg + period + tmp.lag7.max + tmp.lag14.max + tmp.deploy.max + tmp.middle.max + month,
            data = data2)
summary(fit.max.50)
```
...
Just sd
```{r}
fit.sd.50 <- lm(area.50.all.GPS ~ n.days.GPS + Mass_kg + period + tmp.lag7.sd + tmp.lag14.sd + tmp.deploy.sd + tmp.middle.sd + month,
            data = data2)
summary(fit.sd.50)
```

```{r}
fit.month.50 <- lm(area.50.all.GPS ~ month + SCL_cm,
            data = data2)
summary(fit.month.50)
```
