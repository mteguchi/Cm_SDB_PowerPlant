...
if (plot.fig){
  ...
  pre.df <- data.frame(n = nrow(filter(used.n, pre0post1 == 0)),
                       areas = pre.UD$HR$area.50)  
  ...
  p.dif.dist <- ggplot() + 
    ...
    labs(x = "Number of individuals",
         y = bquote('Estimated 50% UDs (' *km^2*')'))
...
Look at how many individuals were available:
```{r}
ID.post.min_n %>% nrow()
ID.pre.min_n %>% nrow()
...
area50.df %>% filter(n == 4) %>% nrow() -> n.n4
n.n4
area50.df %>% filter(n == 4) %>% summarise(min(area50), max(area50), 
                                           mean(area50), sqrt(var(area50)/n.n4),
                                           sqrt(var(area50)))

# the total 50% UD with all individuals:
post.UD$HR$area.50
```
...
length(dif.50.list[[1]]$dif)
sum(dif.50.list[[1]]$dif > 0)
sum(dif.50.list[[1]]$dif > 0)/length(dif.50.list[[1]]$dif)
```

Look at changes for day and night
```{r}
pre.UD.day$HR$area.50
post.UD.day$HR$area.50

pre.UD.night$HR$area.50
post.UD.night$HR$area.50
...
Look at the individual UDs against SSTs

```{r}
data1 <- read.csv(file = "data/IDvsUDs_SST.csv")
data1 %>% dplyr::select(ArgosID, n.days.GPS, n.relocations.all.GPS, date.GPS, Capture_Date,
                        Sex, Mass_kg, SCL_cm, period, area.50.all.GPS, area.95.all.GPS,
                        tmp.lag7.mean, tmp.lag7.max, tmp.lag7.min, tmp.lag7.sd,
                        tmp.lag14.mean, tmp.lag14.max, tmp.lag14.min, tmp.lag14.sd,
                        tmp.deploy.mean, tmp.deploy.max, tmp.deploy.min, tmp.deploy.sd,
                        tmp.middle.mean, tmp.middle.max, tmp.middle.min, tmp.middle.sd,
                        month) %>%
  na.omit() -> data1.2

data2 <- data.frame(scale(select_if(data1.2, is.numeric)) )
data2$sex <- data1.2$Sex
data2$period <- data1.2$period
```


Use Random Forests to find useful variables - just throw in everything! Something is not working here. RF is giving nonsensical results - probably because of the small number of observations (n = 17)...

```{r}
# library(party)
# M1 <- as.formula("area.50.all.GPS ~ n.days.GPS + Mass_kg + month +tmp.lag14.mean + tmp.lag14.min + tmp.lag14.max + tmp.lag14.sd + tmp.lag7.mean + tmp.lag7.min + tmp.lag7.max + tmp.lag7.sd + tmp.deploy.mean + tmp.deploy.min + tmp.deploy.max + tmp.deploy.sd + tmp.middle.mean + tmp.middle.min + tmp.middle.max + tmp.middle.sd")
# 
# if (!file.exists("RData/RF_M1_out.rds")){
#   RF.M1 <- cforest(formula = M1,
#                    data = data1.2, 
#                    control = cforest_unbiased(ntree = 2000, trace = 200))
#   
#   saveRDS(RF.M1, file = "RData/RF_M1_out.rds")
#   
# } else {
#   RF.M1 <- readRDS("RData/RF_M_1_out.rds")
# }
# 
# if (!file.exists("RData/RF_M1_varImp.rds")){
#   var.imp <- sort(varimp(RF.M1), decreasing = FALSE)
#   saveRDS(var.imp, file = "RData/RF_M1_varImp.rds")
# } else {
#   var.imp <- readRDS("RData/RF_M1_varImp.rds")
# }
# 
# if (!file.exists("RData/RF_M1_predict.rds")){
#   pred.RF.M1 <- predict(RF.M1, newdata = NULL)
#   saveRDS(pred.RF.M1, file = "RData/RF_M1_predict.rds")
# } else {
#   pred.RF.M1 <- readRDS("RData/RF_M1_predict.rds")
# }
# 
...
Create AICc function:
```{r}
AICc <- function(x){
  k <- length(coef(x))
  n <- nrow(x$model)
  
  AICc <- AIC(x) + ((2 * k) * (k + 1))/(n - k - 1)
  return(AICc)
}
```
...
fit.lag14.50 <- lm(area.50.all.GPS ~ n.days.GPS + Mass_kg + period + tmp.lag14.mean + tmp.lag14.sd + month,
            data = data1.2)
summary(fit.lag14.50)
```
...
lag 7
```{r}
fit.lag7.50 <- lm(area.50.all.GPS ~ n.days.GPS + Mass_kg + period + tmp.lag7.mean + tmp.lag7.sd + month,
                  data = data1.2)
summary(fit.lag7.50)
...
entire deploy period
```{r}
fit.deploy.50 <- lm(area.50.all.GPS ~ n.days.GPS + Mass_kg + period + tmp.deploy.mean + tmp.deploy.sd + month,
                    data = data1.2)
summary(fit.deploy.50)
...
```{r}
fit.middle.50.1 <- lm(area.50.all.GPS ~ n.days.GPS + Mass_kg + period + tmp.middle.mean + tmp.middle.sd + month,
                      data = data1.2)
summary(fit.middle.50.1)

```

A bit better but still not very useful - interesting that sd of temperature is a little important. 

```{r}
fit.middle.50.2 <- lm(area.50.all.GPS ~ n.days.GPS + period + tmp.middle.mean + tmp.middle.sd + month,
                      data = data1.2)
summary(fit.middle.50.2)

```

```{r}
fit.middle.50.3 <- lm(area.50.all.GPS ~ period + tmp.middle.mean + tmp.middle.sd + month,
                      data = data1.2)
summary(fit.middle.50.3)

```


```{r}
fit.middle.50.4 <- lm(area.50.all.GPS ~ period + tmp.middle.sd + month,
                      data = data1.2)
summary(fit.middle.50.4)

```

So... this model explains about 34% of the variability in the area, although none of the variables were "significant." 


```{r}
fit.middle.50.5 <- lm(area.50.all.GPS ~ tmp.middle.sd + month,
                      data = data1.2)
summary(fit.middle.50.5)

```


```{r}
fit.middle.50.6 <- lm(area.50.all.GPS ~ tmp.middle.sd,
            data = data1.2)
summary(fit.middle.50.6)

```

```{r}
fit.middle.50.7 <- lm(area.50.all.GPS ~ period + tmp.middle.sd * tmp.middle.mean + month,
                      data = data1.2)
summary(fit.middle.50.7)
...
```{r}
fit.middle.50.8 <- lm(area.50.all.GPS ~ period * month + tmp.middle.sd,
                      data = data1.2)
summary(fit.middle.50.8)
...
<!-- ```{r} -->
<!-- data1.2$tmp.middle.dif <- abs(data1.2$tmp.middle.max - data1.2$tmp.middle.min)/data1.2$tmp.middle.mean -->

<!-- fit.middle.50.9 <- lm(area.50.all.GPS ~ tmp.middle.dif*tmp.middle.dif, -->
<!--                       data = data1.2) -->
<!-- summary(fit.middle.50.9) -->

<!-- ``` -->


```{r}
AICs <- c(AICc(fit.middle.50.1), AICc(fit.middle.50.2),
          AICc(fit.middle.50.3), AICc(fit.middle.50.4),
          AICc(fit.middle.50.5), AICc(fit.middle.50.6),
          AICc(fit.middle.50.7), AICc(fit.middle.50.8))

AICs
```

It appears that Model 6 is most parsimonious according to AICc. The model included only SD of temperature at the middle of deployments.  
...
Look into this in a bit more details...

```{r}
ggplot(data = data1.2) + 
  #geom_point(aes(x = tmp.middle.sd, y = area.50.all.GPS)) + 
  geom_text(aes(x = tmp.middle.sd,
                y = area.50.all.GPS,
                label = ArgosID,
...
Not sure what would cause this result... 
...
if (plot.fig){
  ...
  p1 <- ggplot() + 
    geom_polygon(data = SDBay.latlon.df,
                 ...
                 fill = water.color,
                 color = "black") + 
    geom_polygon(data = eelg.2008.latlon.df,
                 aes(x = long, y = lat, group = group),
                 fill = eelgrass.color) + 
    ...
    geom_polygon(data = pre.UD.50.df,
                 ...
                 color = pre.50.color,
                 alpha = 0.4)  +
    geom_line(data = data.frame(long = c(-117.156, -117.117),
                                lat = c(32.66, 32.66)),
              aes(x = long, y = lat),
              size = 1, linetype = 2) +
    geom_polygon(data = SDBay.latlon.df,
...
Compute the total area of overlap between UD50 and eelgrass
```{r}
sum(unlist(pre.UD.50.eelg.all.areas))

# that's in m2
# so convert it in km2
sum(unlist(pre.UD.50.eelg.all.areas))/1e+6

pre.eelgrass.area/1e+6
...
if (plot.fig){
  p1 <- ggplot() + 
   geom_polygon(data = SDBay.latlon.df,
               aes(x = long, y = lat, group = group),
               fill = water.color) + 
  geom_polygon(data = eelg.2014.latlon.df,
               aes(x = long, y = lat, group = group),
               fill = eelgrass.color) + 
...
Compute the total area of overlap between UD50 and eelgrass
```{r}
sum(unlist(post.UD.50.eelg.all.areas))

# that's in m2
# so convert it in km2
sum(unlist(post.UD.50.eelg.all.areas))/1e+6

post.eelgrass.area/1e+6
...
Compute the total area of overlap between UD50 and eelgrass
```{r}
sum(unlist(pre.day.UD.50.eelg.all.areas))

# that's in m2
# so convert it in km2
sum(unlist(pre.day.UD.50.eelg.all.areas))/1e+6
...
Compute the total area of overlap between UD50 and eelgrass
```{r}
sum(unlist(pre.night.UD.50.eelg.all.areas))

# that's in m2
# so convert it in km2
sum(unlist(pre.night.UD.50.eelg.all.areas))/1e+6
...
Compute the total area of overlap between UD50 and eelgrass
```{r}
sum(unlist(post.day.UD.50.eelg.all.areas))

# that's in m2
# so convert it in km2
sum(unlist(post.day.UD.50.eelg.all.areas))/1e+6
...
Compute the total area of overlap between UD50 and eelgrass
```{r}
sum(unlist(post.night.UD.50.eelg.all.areas))

# that's in m2
# so convert it in km2
sum(unlist(post.night.UD.50.eelg.all.areas))/1e+6

post.eelgrass.area/1e+6

```

Compute overlap between day and night:
```{r}
# find the intersections with UDs
pre.day.vs.night.UD.50 <- rgeos::gIntersection(area.pre.night.50,
                                                area.pre.day.50)

pre.day.vs.night.UD.50.latlon <- spTransform(pre.day.vs.night.UD.50,
                                               CRS = CRS("+proj=longlat +datum=WGS84"))

pre.day.vs.night.UD.50.all.areas <- lapply(pre.day.vs.night.UD.50.latlon@polygons[[1]]@Polygons,
                                           FUN = function(x){
                                             areas <- geosphere::areaPolygon(slot(x, "coords")) * 
                                               slot(x, "ringDir")
                                           } )
...
# in km2
sum(unlist(pre.day.vs.night.UD.50.all.areas))/1e+6

post.day.vs.night.UD.50 <- rgeos::gIntersection(area.post.night.50,
                                                area.post.day.50)
post.day.vs.night.UD.50.latlon <- spTransform(post.day.vs.night.UD.50,
                                               CRS = CRS("+proj=longlat +datum=WGS84"))

post.day.vs.night.UD.50.all.areas <- lapply(post.day.vs.night.UD.50.latlon@polygons[[1]]@Polygons,
                                           FUN = function(x){
                                             areas <- geosphere::areaPolygon(slot(x, "coords")) * 
                                               slot(x, "ringDir")
                                           } )

sum(unlist(post.day.vs.night.UD.50.all.areas))/1e+6
