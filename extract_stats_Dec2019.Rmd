...
eelg.2014 <- spTransform(readOGR(dsn = "GISfiles/Features",
                                       layer = "SD_Baywide_Eelgrass_2014_Final",
                                       verbose = F),
                               CRS(slot(pre.UD$HR$ver.50@proj4string, "projargs")))
eelg.2014.df <- broom::tidy(SDBay.eelg.2014)
...
eelg.2008.UTM.south <- spTransform(eelg.2008.latlon.south,
                                   CRS = proj4string(SDBay.eelg.2008))
...
# compute the overlap between UD and eelgrass
pre.eelgrass.UD.50 <- rgeos::gIntersection(eelg.2008.UTM.south, pre.UD$HR$ver.50)
pre.eelgrass.UD.95 <- rgeos::gIntersection(eelg.2008.UTM.south, pre.UD$HR$ver.95)

# create df for plotting
pre.UD.95.df <- broom::tidy(spTransform(pre.UD$HR$ver.95,
                                        CRS = CRS("+proj=longlat +datum=WGS84")))
pre.UD.50.df <- broom::tidy(spTransform(pre.UD$HR$ver.50,
                                        CRS = CRS("+proj=longlat +datum=WGS84")))
...
pre.eelgrass.UD.area <- slot(pre.eelgrass@polygons[[1]], "area")

eelg.2008.df <- broom::tidy(eelg.2008.UTM.south)
pre.UD.df <- broom::tidy(pre.UD$HR$ver.50)

ggplot() + 
  geom_polygon(data = pre.UD.df,
               aes(x = long, y = lat, group = group),
               fill = "black") + 
  geom_polygon(data = eelg.2008.df,
               aes(x = long, y = lat, group = group),
               fill = "green")  
  #coord_map()
