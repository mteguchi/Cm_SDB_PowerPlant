...
# with UDs... so I manually modify it.
tmp2.1 <- tmp2[idx2]
...
# Then, put them together as one.
eelg.2014.latlon.south <- SpatialPolygons(list(Polygons(srl = tmp2.1, ID = "0"),
                                               Polygons(srl = tmp5[idx5], ID = "1")),
...
# compute the eelgrass area
post.eelgrass.area <- slot(eelg.2014.UTM.south@polygons[[1]], "area") + 
  slot(eelg.2014.UTM.south@polygons[[2]], "area")

# extract the land area
area.post.daynight.95 <- get.area.UTM(post.daynight$HR$ver.95)
area.post.daynight.50 <- get.area.UTM(post.daynight$HR$ver.50)

# these lines crash RStudio.
