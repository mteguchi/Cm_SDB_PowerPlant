...
SDBay <- spTransform(readOGR(dsn = "GISfiles",
                             layer = "sd_bay",
                             verbose = FALSE),
                     CRS(slot(pre.UD$HR$ver.50@proj4string, "projargs")))
SDBay.df <- broom::tidy(SDBay)

eelg.2008.latlon <- spTransform(readOGR(dsn = "GISfiles/Features",
...
# So... only 1st and 6th items contain polygons south of N.end...
tmp1 <- eelg.2008.latlon@polygons[[1]]@Polygons
idx1 <- sapply(tmp1, 
              function(x) max(coordinates(x)[,2]) < N.end)

# tmp2 <- eelg.2008.latlon@polygons[[2]]@Polygons
# idx2 <- sapply(tmp2, 
#               function(x) max(coordinates(x)[,2]) < N.end)
# 
# tmp3 <- eelg.2008.latlon@polygons[[3]]@Polygons
# idx3 <- sapply(tmp3, 
#               function(x) max(coordinates(x)[,2]) < N.end)
# 
# tmp4 <- eelg.2008.latlon@polygons[[4]]@Polygons
# idx4 <- sapply(tmp4, 
#               function(x) max(coordinates(x)[,2]) < N.end)
# 
# tmp5 <- eelg.2008.latlon@polygons[[5]]@Polygons
# idx5 <- sapply(tmp5, 
#               function(x) max(coordinates(x)[,2]) < N.end)

tmp6 <- eelg.2008.latlon@polygons[[6]]@Polygons
idx6 <- sapply(tmp6, 
              function(x) max(coordinates(x)[,2]) < N.end)

# Then, put them together as one.
eelg.2008.latlon.south <- SpatialPolygons(list(Polygons(srl = tmp1[idx1], ID = "0"),
                                               Polygons(srl = tmp6[idx6], ID = "1")),
                                          proj4string=CRS(proj4string(eelg.2008.latlon)))
...
eelg.2008.UTM.south <- spTransform(eelg.2008.latlon.south,
                                   CRS = proj4string(pre.daynight$HR$ver.50))

eelg.2008.UTM.south.df <- broom::tidy(eelg.2008.UTM.south)

# compute the eelgrass area
pre.eelgrass.area <- slot(eelg.2008.UTM.south@polygons[[1]], "area") + 
  slot(eelg.2008.UTM.south@polygons[[2]], "area")
...
pre.eelgrass.UD.50 <- rgeos::gIntersection(eelg.2008.UTM.south, area.pre.daynight.50)
pre.eelgrass.UD.95 <- rgeos::gIntersection(eelg.2008.UTM.south, area.pre.daynight.95)
...
Post period:
```{r}
eelg.2014.latlon <- spTransform(readOGR(dsn = "GISfiles/Features",
...
eelg.2014.UTM <- spTransform(eelg.2014.latlon, 
                             CRS = CRS("+proj=utm +zone=11 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0"))

# there is/are polygon(s) that intersects with 32.66 (N.end), and hole(s) that does not.
# so just eliminating polygons that max(lat) < N.end creates orphan holes. To avoid it,
# I convert all latitudes >= 32.66 to 32.659. Then, the intersecting polygon(s) will be
# truncated at 32.66 with holes inside. (I think...)

# layers 2 and 5 contain eelgrass patches. 

# tmp1 <- eelg.2014.latlon@polygons[[1]]@Polygons
# idx1 <- sapply(tmp1, 
#               function(x) max(coordinates(x)[,2]) < N.end)

tmp2 <- eelg.2014.latlon@polygons[[2]]@Polygons
for (k in 1:length(tmp2)){
  tmp <- coordinates(tmp2[[k]])
  tmp[tmp[, 2] > N.end, 2] <- N.end
  slot(tmp2[[k]], "coords") <- tmp
}

# this removes all polygons that were north of N.end because latitude that were
# greater than N.end was set to N.end.

idx2 <- sapply(tmp2,
               function(x) var(coordinates(x)[,2]) > 0)

#also... find the first TRUE in idx2
#idx.first.T <- min(seq(from = 1, to = length(idx2))[cumsum(idx2) == 1])
# this returns 1154 - this is the one that gets fixed in tmp2.1 below.

# negate idx2
#a_idx2 <- !(idx2)

# tmp3 <- eelg.2014.latlon@polygons[[3]]@Polygons
# idx3 <- sapply(tmp3,
#               function(x) max(coordinates(x)[,2]) < N.end)
# 
# tmp4 <- eelg.2014.latlon@polygons[[4]]@Polygons
# idx4 <- sapply(tmp4,
#               function(x) max(coordinates(x)[,2]) < N.end)

# all polygons in [[5]] are south of N.end
tmp5 <- eelg.2014.latlon@polygons[[5]]@Polygons

# for (k in 1:length(tmp5)){
#   tmp <- coordinates(tmp5[[k]])
#   tmp[tmp[, 2] > N.end, 2] <- N.end
#   slot(tmp5[[k]], "coords") <- tmp
# }
# 
# idx5 <- sapply(tmp5,
#                function(x) var(coordinates(x)[,2]) > 0)
...
# the first polygon of tmp2 is cut off by the N.end value, making a flat line at N.end
# from -117.16 to about -117.1523. This causes a problem when trying to find intersections
# with UDs... so I manually modify it. Did the area fixed also? No. --- need to fix it. 
tmp2.1 <- tmp2[idx2]
coord.2.1 <- coordinates(tmp2.1[[1]])
coord.2.1[coord.2.1[, 1] < -117.1523, ] <- NA
coord.2.1 <- na.omit(coord.2.1)
area.2.1 <- geosphere::areaPolygon(coord.2.1)

# set the coordinates with the modified coordinates. I can't replace the area
# here because the area should be in a different unit from square meters, which 
# is the unit of output from areaPolygon.  I don't know what the unit is... this needs
# to be figured out... FIX THIS! 1/8/2020
slot(tmp2.1[[1]], "coords") <- coord.2.1

#tmp2.2 <- tmp2[a_idx2]

# Then, put them together as one. this doesn't work because of the tmp2.1... 
# However... the problem polygon does not overlap with UDs. So... to make things simpler, 
# I'll keep it for plotting purposes but remove it from computing the overlaps.
# the area of the modified polygon tmp2.1[[1]] or eelg.2014.latlon.south.1@polygons[[1]]@Polygons[[1]]
# is not changed from the original size because that was not modified above... So... I need to do 
# something about that. 
eelg.2014.latlon.south.1 <- SpatialPolygons(list(Polygons(srl = tmp2.1, ID = "0"),
                                                 Polygons(srl = tmp5, ID = "1")),
                                            proj4string=CRS(proj4string(eelg.2014.latlon)))

eelg.2014.latlon.south.df <- broom::tidy(eelg.2014.latlon.south.1)

# The following works with gIntersection
eelg.2014.latlon.south.2 <- SpatialPolygons(list(Polygons(srl = tmp5, ID = "0")),
                                            proj4string=CRS(proj4string(eelg.2014.latlon)))

eelg.2014.latlon.south.2.df <- broom::tidy(eelg.2014.latlon.south.2)

##  for checking areas
eelg.2014.latlon.south.3 <- SpatialPolygons(list(Polygons(srl = tmp2.1, ID = "0")),
                                            proj4string=CRS(proj4string(eelg.2014.latlon)))

eelg.2014.UTM.south.3 <- spTransform(eelg.2014.latlon.south.3, 
                                     CRS = CRS("+proj=utm +zone=11 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0"))
eelg.2014.latlon.south.3.df <- broom::tidy(eelg.2014.latlon.south.3)
### 
...
# areas are computed using UTM coordinates to keep the same unit - shouldn't make any difference
# but just to be sure.
eelg.2014.UTM.south.1 <- spTransform(eelg.2014.latlon.south.1,
                                     CRS = proj4string(post.daynight$HR$ver.50))

# Area is measured in square meters so it needs to be placed here rather than when 
# unit was lat/lon - but this would not change the total area...  
slot(eelg.2014.UTM.south.1@polygons[[1]]@Polygons[[1]], "area") <- area.2.1

# compute the eelgrass area - make sure area is correct with removed polygons!! 1/8/2020!!
post.eelgrass.area <- slot(eelg.2014.UTM.south.1@polygons[[1]], "area") + 
  slot(eelg.2014.UTM.south.1@polygons[[2]], "area")

#eelg.2014.UTM.south.df <- broom::tidy(eelg.2014.UTM.south)

# To compute the overlap, use .2 file
eelg.2014.UTM.south.2 <- spTransform(eelg.2014.latlon.south.2,
                                     CRS = proj4string(post.daynight$HR$ver.50))

# these lines crash RStudio. They don't when tmp2.1 is taken out.
post.eelgrass.UD.50 <- rgeos::gIntersection(eelg.2014.UTM.south.2, 
                                            area.post.daynight.50)
post.eelgrass.UD.95 <- rgeos::gIntersection(eelg.2014.UTM.south.2,
...
# ggsave(filename = "figures/post_eelgrass_with_tmp21.png",
#        device = "png",
#        plot = p1)
...
area.pre.day.95 <- spTransform(get.area.UTM(pre.day$HR$ver.95), 
                               CRS=CRS("+proj=longlat +datum=WGS84"))
area.pre.night.95 <- spTransform(get.area.UTM(pre.night$HR$ver.95),
                                 CRS=CRS("+proj=longlat +datum=WGS84"))
area.pre.day.50 <- spTransform(get.area.UTM(pre.day$HR$ver.50), 
                               CRS=CRS("+proj=longlat +datum=WGS84"))
area.pre.night.50 <- spTransform(get.area.UTM(pre.night$HR$ver.50),
                                 CRS=CRS("+proj=longlat +datum=WGS84"))
```
