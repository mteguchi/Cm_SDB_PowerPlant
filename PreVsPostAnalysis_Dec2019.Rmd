...
Load results from HR_analysis_GPS_LC_newtags_2019-10-31.Rmd. This was done to use just GPS data, which was suggested by a reviewer.  The following files contain raw data for pre and post periods.
...
pre.HR <- compute.area(h = h.pre, 
                       all.utm = pre.kd$list.data$all.utm, 
                       grid = grid)
...
post.HR <- compute.area(h = h.post, 
                       all.utm = post.kd$list.data$all.utm, 
                       grid = grid)
...
for (k in 1:length(file.names)){
  ...
  dif.50.list[[as.numeric(n.txt) - 4]] <- data.frame(dif = areas.combos$area50 - pre.HR$area.50)
  dif.95.list[[as.numeric(n.txt) - 4]] <- data.frame(dif = areas.combos$area95 - pre.HR$area.95)
...
Even at sample sie of 5, the difference between the mean of 1000 UDs and the one from 16 samples was pretty small.
```{r}
...
So, the majority of the differences are above zero (99%), indicating that there was an increase in 50% UDs after the PP shut down. 
